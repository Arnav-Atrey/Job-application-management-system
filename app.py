...
def profile():
    if "user" in session:
        ...
        cur2.execute("SELECT jobseeker_id, college, department,education FROM profile WHERE jobseeker_id = {}".format(user))
...
def manageprofile():
    if "user" in session:
        ...
        if exist > 0:
            # If the profile entry for the user exists, fetch the profile data
            profile_data = cur.fetchall()[-1]
        else:
            # If no profile entry exists, set profile_data to None or an empty dictionary
            profile_data = None
        ...
        if request.method == 'POST':
            ...
            if exist > 0:
                # If the profile entry for the user exists, update the profile details
                cur.execute(
                    "UPDATE profile SET college = (%s), department = (%s), education = (%s) WHERE jobseeker_id = (%s)",
                    (college, dept, education, user)
                )
                mysql.connection.commit()
            else:
                # If no profile entry exists, insert a new record for the profile details
                cur.execute(
                    "INSERT INTO profile(college, department, education, jobseeker_id) VALUES (%s, %s, %s, %s)",
                    (college, dept, education, user)
                )
                mysql.connection.commit()
        ...
        flash('Data could not be entered....', 'danger')
        return redirect(url_for('login'))
...
def apply():
    if "user" in session:
        ...
        if request.method == 'POST':
            ...
            # Check if the jobseeker has already applied for the job
            applied = cur.execute("SELECT * FROM apply WHERE (jobseeker_id, job_id) = ({}, {})".format(user, job_id))
            if applied == 0:
                # If not, insert a record for the job application
                cur.execute("INSERT INTO apply VALUES ({}, {})".format(user, job_id))
                mysql.connection.commit()
                ...
                # Flash an error message if already applied
